...
export async function createFeedback(userId, userName, type, message, experimentId, experimentName, rating) {
    return db.run(`INSERT INTO feedback (user_id, user_name, type, experiment_id, experiment_name, rating, message)
     VALUES (?, ?, ?, ?, ?, ?, ?)`, userId, userName, type, experimentId || null, experimentName || null, rating || null, message);
}
export async function getAllFeedback() {
    return db.all(`SELECT * FROM feedback ORDER BY created_at DESC LIMIT 200`);
}

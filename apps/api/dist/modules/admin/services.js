...
export async function getAllUsers() {
    return db.all(`SELECT id, email, name, role, email_verified_at, created_at FROM users ORDER BY created_at DESC`);
}
...
export async function getAllClassesWithTeachers() {
    return db.all(`SELECT c.*, u.name as teacher_name, u.email as teacher_email,
...
export async function getAllReportsWithDetails(page = 1, limit = 50) {
    ...
    return db.all(`SELECT r.*, u.name as student_name, u.email as student_email,
     c.name as class_name, t.name as teacher_name
     FROM experiment_reports r
     JOIN users u ON r.student_id = u.id
     JOIN classes c ON r.class_id = c.id
     JOIN users t ON c.teacher_id = t.id
     ORDER BY r.submitted_at DESC LIMIT ? OFFSET ?`, limit, offset);
}

...
export async function getStats() {
    ...
    const pendingReports = await db.get(`SELECT COUNT(*) as count FROM experiment_reports WHERE status = 'submitted'`);
    const gradedReports = await db.get(`SELECT COUNT(*) as count FROM experiment_reports WHERE status = 'graded'`);
    return {

...
export async function getTeacherStats(teacherId: number): Promise<TeacherLiveStats> {
  const row = await db.get<{
    ...
    `SELECT
       ...
       SUM(CASE WHEN date(r.submitted_at, 'localtime') = date('now', 'localtime') THEN 1 ELSE 0 END) as submitted_today,
       SUM(CASE WHEN r.graded_at IS NOT NULL AND date(r.graded_at, 'localtime') = date('now', 'localtime') THEN 1 ELSE 0 END) as graded_today

...
// Escape user-controlled text before interpolating it into HTML emails or
// Telegram messages sent with HTML parse mode (stored-XSS / markup breakage).
...
export async function sendVerificationEmail(email: string, name: string, code: string): Promise<boolean> {
  const html = `
    <div dir="rtl" style="font-family: system-ui, sans-serif; max-width: 480px; margin: 0 auto; padding: 2rem;">
      <h2 style="color: #6366f1;">${escapeHtml(APP_NAME)}</h2>
...
export async function sendComplaintAlert(data: {
  ...
  const cat = CATEGORY_LABELS[data.category] || escapeHtml(data.category);
  const pri = PRIORITY_LABELS[data.priority] || escapeHtml(data.priority);
  const roleLabel = data.fromRole === 'student' ? 'طالب' : data.fromRole === 'teacher' ? 'مدرس' : escapeHtml(data.fromRole);
  const safeName = escapeHtml(data.fromName);
  ...
  const html = `
    <div dir="rtl" style="font-family: system-ui, sans-serif; max-width: 560px; margin: 0 auto; padding: 2rem;">
      ...
      <table style="width:100%; border-collapse:collapse; margin:1rem 0;">
        <tr><td style="padding:6px; color:#64748b;">المُرسِل</td><td style="padding:6px; font-weight:600;">${safeName} (${roleLabel})</td></tr>
        ...
        <tr><td style="padding:6px; color:#64748b;">الموضوع</td><td style="padding:6px; font-weight:600;">${safeSubject}</td></tr>
      </table>
      ...
      <p style="color:#94a3b8; font-size:0.8rem; margin-top:1rem;">${escapeHtml(APP_NAME)} — نظام الشكاوى</p>
    </div>
  `;
...
export async function sendComplaintToTarget(data: {
  ...
  const html = `
    <div dir="rtl" style="font-family: system-ui, sans-serif; max-width: 560px; margin: 0 auto; padding: 2rem;">
      ...
      <p>مرحباً ${escapeHtml(data.targetName)}،</p>

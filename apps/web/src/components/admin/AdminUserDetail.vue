...
async function onImpersonate() {
  ...
  try {
    ...
    if (res.success && res.token) { setTokens(res.token); localStorage.setItem('auth_user', JSON.stringify(res.user)); localStorage.removeItem('school_session'); window.location.href = `/#/${res.user.role}`; }
  } catch {

...
const pvCalc = computed(() => props.p * props.v)
...
<template>
  <div class="panel-body">
    <div class="ws-title">📐 خطوات قانون بويل</div>

    <div class="step-row">
      <button class="step-toggle" @click="toggleStep('step1')">
        {{ showSteps.step1 ? '▼' : '▶' }} الخطوة 1: حساب P×V
      </button>
      <div v-if="showSteps.step1" class="step-content">
        <div class="formula">P × V = constant</div>
        <div class="calc">
          <div>P × V = {{ p }} atm × {{ v }} L</div>
          <div class="result">P·V = {{ pvCalc.toFixed(2) }} atm·L</div>
        </div>
      </div>
    </div>
  </div>

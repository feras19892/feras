...
import {
  getLiquid, getBurette, getPipette, getSepFunnelState, getBurnerState, getBalanceTare,
...
const props = defineProps<{
  ...
  cursorPipette: { item: LabItem; state: { volume: number; maxVolume: number; color: string } } | null;
  canUndo?: boolean;
  canRedo?: boolean;
}>();

const emit = defineEmits<{
  action: [type: 'refill' | 'empty' | 'toggleValve' | 'fill50' | 'fill100' | 'remove50' | 'remove100', uid: string];
  remove: [uid: string];
  enterPipette: [];
  exitPipette: [];
  toggleBurner: [];
...
function pipetteStatus(): string {
  if (!props.item) return '';
  if (props.cursorPipette) return '💉 وضع التحكم نشط';
  ...
  <div v-if="item && state" class="floating-inspector">
    ...
    <div class="fi-body">
      ...
      <template v-if="state.type === 'beaker'">
        ...
        <template v-if="item">
          <div class="fi-row"><span>🌡️ الحرارة</span><b :class="getLiquid(item.uid).temperature > 30 ? 'hot' : ''">{{ getLiquid(item.uid).temperature.toFixed(1) }}°C</b></div>
          <div class="fi-row"><span>📟 pH</span><b :class="getLiquid(item.uid).ph !== null ? (getLiquid(item.uid).ph! < 7 ? 'acid' : getLiquid(item.uid).ph! > 7 ? 'base' : 'neutral') : ''">{{ getLiquid(item.uid).ph !== null ? getLiquid(item.uid).ph!.toFixed(2) : '--.--' }}</b></div>
          <div class="fi-row"><span>🔥 التسخين</span><b>{{ getLiquid(item.uid).heated ? '🟡 نعم' : '⚪ لا' }}</b></div>
        </template>
        <div class="fi-row">
          <span>المحلول</span>
          <input
      ...
      <template v-else-if="state.type === 'pipette'">
        ...
        <div class="fi-row"><span>اللون</span><span class="fi-dot" :style="{ background: state.color }" /></div>
        <div class="fi-actions">
          <button class="success" @click="emit('enterPipette')">{{ state.volume <= 0 ? '💉 وضع السحب' : '💉 وضع التفريغ' }}</button>
        </div>
        <div class="fi-actions">
          <button class="delete" @click="emit('remove', item.uid)">❌ إزالة</button>
        </div>
      ...
      <template v-else-if="item.id === 'digital-balance'">
        ...
        <div class="fi-actions">
          <button class="success" @click="emit('tare')">⚖️ تصفير الوزن (TARE)</button>
        </div>
        <div class="fi-actions">
          <button class="delete" @click="emit('remove', item.uid)">❌ إزالة</button>
        </div>
      ...
      <template v-else-if="item.id === 'pipette'">
        ...
        <div class="fi-row hint"><span>📖</span><small>اضغط على الماصة → مرِّر فوق حاوية → انقر لسحب/إفراغ</small></div>
        <div class="fi-actions">
          <button v-if="!cursorPipette" class="success" @click="emit('enterPipette')">💉 دخول وضع التحكم</button>
          <button v-else class="danger" @click="emit('exitPipette')">⏹️ خروج من وضع التحكم</button>
        </div>
        <div class="fi-actions">
          <button class="delete" @click="emit('remove', item.uid)">❌ إزالة</button>
        </div>

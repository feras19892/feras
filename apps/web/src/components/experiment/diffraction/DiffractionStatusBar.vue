...
<template>
  <div class="status-bar" :class="stateClass">
    ...
    <span class="sep" />
    <span v-if="mode === 'single'" class="st-item">a = <b>{{ slitWidth.toFixed(2) }}</b> mm</span>
    <span v-else class="st-item">N = <b>{{ linesPerMm }}</b> lines/mm</span>
    ...
    <span v-if="mode === 'single'" class="st-item">w = <b class="green">{{ centralWidth.toFixed(3) }}</b> mm</span>
    <span v-else class="st-item">θ₁ = <b class="green">{{ firstOrderAngle.toFixed(3) }}</b>°</span>
  </div>
</template>

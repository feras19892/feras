...
<template>
  <div class="panel-body">
    <!-- Single slit: y vs 1/a Regression Chart -->
    <div v-if="mode === 'single' && chartPoints.length >= 2" class="chart-box">
    ...
    <div v-else-if="mode === 'single' && intensityPattern.length" class="mini-chart">
    ...
    <div v-else-if="mode === 'grating' && orderPositions.length" class="mini-chart">
      <svg viewBox="0 0 100 40" preserveAspectRatio="none" class="chart-svg">
        <polyline
          :points="orderPositions.map((p, i) => `${(i / (orderPositions.length - 1)) * 100},${40 - p.intensity * 35}`).join(' ')"
          fill="none" stroke="#5B8DB8" stroke-width="0.5"
        />
      </svg>
    </div>

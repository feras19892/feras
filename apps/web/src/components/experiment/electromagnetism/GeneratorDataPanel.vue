...
import type { GeneratorTrial } from '../../../composables/electromagnetism/useGeneratorExperiment'

defineProps<{
  trials: GeneratorTrial[]
}>()
    ...
    <table v-else class="trial-table">
      ...
      <tbody>
        <tr v-for="trial in trials" :key="trial.id">
          ...
          <td>{{ trial.N }}</td>
          <td>{{ trial.omega.toFixed(1) }}</td>
          <td>{{ trial.emfPeak.toExponential(2) }}</td>
          <td>{{ trial.iPeak.toExponential(2) }}</td>
          <td>{{ trial.frequency.toFixed(2) }}</td>
          <td>{{ trial.power.toExponential(2) }}</td>

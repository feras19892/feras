...
import type { LorentzForceTrial } from '../../../composables/electromagnetism/useLorentzForceExperiment'
import { useI18n } from '../../../composables/useI18n'
const { t } = useI18n()

defineProps<{
  trials: LorentzForceTrial[]
}>()
    ...
    <table v-else class="trial-table">
      ...
      <tbody>
        <tr v-for="trial in trials" :key="trial.id">
          <td>{{ trial.id }}</td>
          <td>{{ trial.V.toFixed(0) }}</td>
          <td>{{ trial.I.toFixed(2) }}</td>
          <td>{{ trial.B.toExponential(2) }}</td>
          <td>{{ isFinite(trial.r) ? trial.r.toExponential(2) : '∞' }}</td>
          <td>{{ trial.em > 0 ? trial.em.toExponential(2) : '—' }}</td>

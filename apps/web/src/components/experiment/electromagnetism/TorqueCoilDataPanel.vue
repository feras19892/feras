...
import type { TorqueCoilTrial } from '../../../composables/electromagnetism/useTorqueCoilExperiment'

defineProps<{
  trials: TorqueCoilTrial[]
}>()
    ...
    <table v-else class="trial-table">
      ...
      <tbody>
        <tr v-for="trial in trials" :key="trial.id">
          ...
          <td>{{ trial.N }}</td>
          <td>{{ trial.B.toFixed(3) }}</td>
          <td>{{ trial.theta.toFixed(0) }}°</td>
          <td>{{ trial.mu.toExponential(2) }}</td>
          <td>{{ trial.tau.toExponential(2) }}</td>

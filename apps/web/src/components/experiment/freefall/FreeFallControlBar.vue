...
<template>
  <div class="control-bar">
    ...
    <button class="btn-noise" :class="{ active: enableNoise }" @click="$emit('toggleNoise')" :title="enableNoise ? 'إطفاء الخطأ النسبي (قياسات دقيقة)' : 'تفعيل الخطأ النسبي'">
      <span class="led" :class="{ on: enableNoise }"></span>
      <span class="noise-label">{{ enableNoise ? 'خطأ نسبي' : 'دقيق' }}</span>
    </button>

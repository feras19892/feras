...
<template>
  <div class="trials-panel">
    <table v-if="trials.length">
      <thead><tr><th>#</th><th>n</th><th>T</th><th>V</th><th>P</th><th></th></tr></thead>
      <tbody>
        <tr v-for="trial in trials" :key="trial.id">
          <td>{{ trial.id }}</td>
          <td>{{ trial.n.toFixed(2) }}</td>
          <td>{{ trial.T }}</td>
          <td>{{ (trial.V*1000).toFixed(1) }}</td>
          <td>{{ (trial.P/1000).toFixed(1) }}</td>
...
th, td { padding:.25rem .4rem; text-align:center; border-bottom:1px solid #1e2530; }
th { color:#5B8DB8; font-weight:700; }
.rm { background:none; border:none; color:#8B95A5; cursor:pointer; font-size:.7rem; }

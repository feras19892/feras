...
function resizeCanvas() {
  const canvas = fftCanvasRef.value, wrap = fftWrapRef.value
...
function draw() {
  ...
  const theoreticalF = props.params.length>0&&props.params.g>0 ? (1/(2*Math.PI))*Math.sqrt(props.params.g/props.params.length) : null
...
onMounted(() => { if (fftWrapRef.value) { resizeObs = new ResizeObserver(() => { resizeCanvas(); draw() }); resizeObs.observe(fftWrapRef.value) } draw() })
...
<template>
  <div class="fft-panel">
    ...
    <div class="fft-canvas-wrap" ref="fftWrapRef"><canvas ref="fftCanvasRef" class="fft-canvas" /></div>
    <div v-if="fftResult" class="fft-result-row">
      <span class="fft-badge dom">f<sub>dom</sub> = {{ fftResult.dominantFreq.toFixed(4) }} Hz</span>
      <span class="fft-badge theo">f₀ = {{ ((1/(2*Math.PI))*Math.sqrt(params.g/params.length)).toFixed(4) }} Hz</span>
      <span class="fft-badge err">Δf = {{ (Math.abs(fftResult.dominantFreq - (1/(2*Math.PI))*Math.sqrt(params.g/params.length)) / ((1/(2*Math.PI))*Math.sqrt(params.g/params.length)) * 100).toFixed(2) }}%</span>
    </div>
  </div>
</template>

...
function start() {
  ...
  count.value = 0; isCounting.value = true; startTime.value = performance.now(); elapsed.value = 0; lastTheta.value = props.simTheta; tick()
}

function stop() { isCounting.value = false; if (rafId) cancelAnimationFrame(rafId); rafId = null }

function resetAll() {
  stop(); count.value = 0; elapsed.value = 0; t1.value = 0; t2.value = 0; t3.value = 0; currentTrial.value = 0
}

function recordTrial() {
  const t = elapsed.value / 1000
...
watch(() => props.simRunning, (running) => { if (!running && isCounting.value) stop() })

watch(() => props.simTheta, (newTheta) => {
  if (!isCounting.value || !props.simRunning) return
  if (lastTheta.value > 0 && newTheta <= 0) {
    count.value++
    if (count.value >= props.targetCount) { elapsed.value = performance.now() - startTime.value; stop(); recordTrial() }
  }
  lastTheta.value = newTheta
}, { immediate: false })

...
const scatterYKey = ref<'length'|'g'|'T'|'T2'|'f'|'omega'>('T2')
...
function calcSlope() { if (fit.value) emit('calcSlope', fit.value.slope, fit.value.intercept, fit.value.r2) }
  ...
  <div class="scatter-panel">
    <div class="card-header">
      ...
      <div class="scatter-axis-controls">
        <select v-model="scatterXKey"><option value="length">L (m)</option><option value="g">g (m/s²)</option><option value="T">T (s)</option><option value="f">f (Hz)</option><option value="omega">ω (rad/s)</option></select>
        <span class="axis-sep">×</span>
        <select v-model="scatterYKey"><option value="length">L (m)</option><option value="g">g (m/s²)</option><option value="T">T (s)</option><option value="T2">T² (s²)</option><option value="f">f (Hz)</option><option value="omega">ω (rad/s)</option></select>
      </div>
    </div>
    ...
    <div class="scatter-footer">
      ...
      <div v-if="fit" class="fit-readout">
        <span>m = {{ fit.slope.toFixed(4) }}</span><span>b = {{ fit.intercept.toFixed(4) }}</span><span>R² = {{ fit.r2.toFixed(4) }}</span>
      </div>
    </div>
  </div>
</template>

...
<template>
  <div class="status-bar" :class="stateClass">
    ...
    <span class="sep" />
    <span class="st-item">θ₁ = <b class="cyan">{{ polarizerAngle }}</b>°</span>
    <span class="st-item">θ₂ = <b class="amber">{{ analyzerAngle }}</b>°</span>
    <span class="st-item">I = <b class="green">{{ outputIntensity.toFixed(1) }}</b></span>
    <span class="st-item">T = <b class="green">{{ transmissionPercent.toFixed(1) }}</b>%</span>
  </div>
</template>

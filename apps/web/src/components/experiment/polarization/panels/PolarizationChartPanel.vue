...
const chartBounds = computed(() => {
  ...
  if (pts.length === 0) return { xMin: 0, xMax: 1.1, yMin: 0, yMax: props.intensityCurve.length ? 1.2 : 120 }
  const xs = pts.map(p => p.cos2)
  const ys = pts.map(p => p.I)
  const xMin = 0
    ...
    <div v-if="chartPoints.length >= 2" class="chart-box">
      <svg :viewBox="`0 0 ${svgW} ${svgH}`" class="reg-chart">
        ...
        <text v-for="i in 5" :key="'xl' + i" :x="toSvgX(chartBounds.xMin + (chartBounds.xMax - chartBounds.xMin) * (i - 1) / 4)" :y="svgH - 8" fill="#8B95A5" font-size="8" text-anchor="middle">
          {{ (chartBounds.xMin + (chartBounds.xMax - chartBounds.xMin) * (i - 1) / 4).toFixed(2) }}
        </text>
      ...
      <div class="reg-summary">
        <span class="reg-badge">Slope = {{ regressionSlope.toFixed(2) }}</span>
        <span class="reg-badge">R² = {{ rSquared.toFixed(4) }}</span>
      </div>
    </div>
    <!-- Fallback intensity curve -->
    <div v-else-if="intensityCurve.length" class="mini-chart">
      <svg viewBox="0 0 100 40" preserveAspectRatio="none" class="chart-svg">
        <polyline
          :points="intensityCurve.map(p => `${(p.theta / 360) * 100},${40 - p.intensity * 35}`).join(' ')"

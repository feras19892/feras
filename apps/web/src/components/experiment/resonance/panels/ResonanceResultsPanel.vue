...
const { t } = useI18n()
interface Props { trials: ResonanceTrial[] }
defineProps<Props>()
</script>
...
<template>
  <div class="panel-body">
    <div v-if="trials.length >= 2" class="results">
      <div class="res-row"><span class="res-label">{{ t('experiments.reTrials') }}</span><span class="res-val">{{ trials.length }}</span></div>
      <div class="res-row"><span class="res-label">{{ t('experiments.reAvgFreq') }}</span><span class="res-val green">{{ (trials.reduce((s, tr) => s + tr.frequency, 0) / trials.length).toFixed(1) }} Hz</span></div>
      <div class="res-row"><span class="res-label">{{ t('experiments.reAvgLambda') }}</span><span class="res-val">{{ (trials.reduce((s, tr) => s + tr.wavelength, 0) / trials.length).toFixed(3) }} m</span></div>
    </div>

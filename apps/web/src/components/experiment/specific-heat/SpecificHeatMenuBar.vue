    ...
    <div class="menu-left">
      ...
      <div class="menu-group">
        ...
        <div v-if="activeMenu==='view'" class="menu-dropdown" @click.stop>
          ...
          <div class="menu-row check" @click="emit('togglePanel','chart')"><span class="mi">&#x1F4C8;</span><span>{{ t('experiments.panelChart') }}</span></div>
          ...
          <div class="menu-row check" @click="emit('togglePanel','laws')"><span class="mi">&#x2696;&#xFE0F;</span><span>{{ t('experiments.panelLaws') }}</span></div>
          <div class="menu-row check" @click="emit('togglePanel','results')"><span class="mi">&#x1F4CB;</span><span>{{ t('experiments.panelResults') }}</span></div>
        </div>
      </div>
    </div>
    <div class="menu-center">{{ icon || '&#x1F321;' }} {{ title }}</div>
    <div class="menu-right">
      <div class="menu-group"><button v-if="experimentRoute" class="menu-btn" @click="router.push(experimentRoute)">{{ icon || '&#x1F321;' }} {{ experimentName || title }}</button></div>
      <div class="menu-group">
      ...
      <div class="menu-group"><button class="menu-btn analyze-btn" @click="emit('analyzeResults')">&#x1F4CA; {{ t('experiments.drawingCalculationsSection') }}</button></div>
      <div class="menu-group"><button class="menu-btn" @click="emit('toggleHelp')">&#x2753; {{ t('experiments.menuHelp') }}</button></div>
    </div>
  </nav>
</template>

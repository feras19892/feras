...
<template>
  <div class="status-bar" :class="stateClass">
    ...
    <span class="sep" />
    <span class="st-item">L = <b class="cyan">{{ tubeLength.toFixed(3) }}</b> m</span>
    <span class="st-item">f = <b class="amber">{{ frequency }}</b> Hz</span>
    <span class="st-item">v = <b class="green">{{ vMeasured.toFixed(1) }}</b> m/s</span>
    <span class="st-item">vₜ = <b>{{ vTheory.toFixed(1) }}</b> m/s</span>
    <span class="st-item">err = <b class="amber">{{ percentError.toFixed(1) }}</b>%</span>
  </div>
</template>

...
interface Props {
  ...
  params: { lensType: 'convex' | 'concave'; focalLength: number; objectDistance: number; objectHeight: number }
...
const emit = defineEmits<{
  ...
  (e: 'clear'): void
  (e: 'update:params', params: { lensType: 'convex' | 'concave'; focalLength: number; objectDistance: number; objectHeight: number }): void
}>()
    ...
    <template v-if="id === 'laws'">
      <div class="law-box">
        ...
        <div class="law-calc" v-if="imageDistance !== null && imageDistance !== 0">
          {{ (1/(params.lensType === 'concave' ? -params.focalLength : params.focalLength)).toFixed(4) }} = {{ (1/params.objectDistance).toFixed(4) }} + {{ (1/imageDistance).toFixed(4) }} = {{ ((1/params.objectDistance) + (1/imageDistance)).toFixed(4) }} ✓
        </div>
      </div>
      <div class="law-box">
...
.lens-toggle { display: grid; grid-template-columns: 1fr 1fr; gap: .3rem; }
.lens-toggle button { padding: .35rem; border-radius: 6px; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.03); color: #8B95A5; font-size: .78rem; cursor: pointer; font-family: inherit; transition: all .15s; }
.lens-toggle button:hover { background: rgba(255,255,255,0.06); }
.lens-toggle button.active { background: rgba(91,141,184,.15); border-color: #5B8DB8; color: #5B8DB8; font-weight: 700; }
</style>

...
<template>
  <div class="status-bar" :class="stateClass">
    ...
    <span class="sep" />
    <span class="st-item">d = <b class="cyan">{{ sourceDistance.toFixed(3) }}</b> m</span>
    <span class="st-item">lambda = <b class="amber">{{ (wavelength * 1000).toFixed(1) }}</b> mm</span>
    <span class="st-item">f = <b class="green">{{ frequency }}</b> Hz</span>
    <span class="st-item">v = <b>{{ vWave.toFixed(1) }}</b> m/s</span>
  </div>
</template>

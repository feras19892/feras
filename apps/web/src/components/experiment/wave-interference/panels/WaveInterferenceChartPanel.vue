...
<template>
  <div class="panel-body">
    <div class="mini-chart" v-if="amplitudeMap.length">
      <svg viewBox="0 0 100 40" preserveAspectRatio="none" class="chart-svg">
        <polyline :points="amplitudeMap.map((p, i) => `${(i / (amplitudeMap.length - 1)) * 100},${20 - p.amplitude * 8}`).join(' ')" fill="none" stroke="#5B8DB8" stroke-width="0.5" />
      </svg>
    </div>

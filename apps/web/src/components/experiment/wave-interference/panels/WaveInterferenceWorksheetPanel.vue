...
const vWave = computed(() => props.frequency * props.wavelength)
const fringeSpacing = computed(() => (props.wavelength * props.screenDistance) / props.sourceDistance)
...
<template>
  <div class="panel-body">
    <div class="ws-title">📐 خطوات تداخل الموجات</div>

    <div class="step-row">
      <button class="step-toggle" @click="toggleStep('step1')">
        {{ showSteps.step1 ? '▼' : '▶' }} الخطوة 1: سرعة الموجة والأهداب
      </button>
      <div v-if="showSteps.step1" class="step-content">
        <div class="formula">v = f·λ</div>
        <div class="calc">
          <div>v = {{ frequency }} Hz × {{ wavelength.toFixed(3) }} m</div>
          <div class="result">v = {{ vWave.toFixed(2) }} m/s</div>
          <div>Δy = λD/d = {{ fringeSpacing.toFixed(4) }} m</div>
        </div>
      </div>
    </div>
  </div>

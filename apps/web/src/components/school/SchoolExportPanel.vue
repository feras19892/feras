...
const types = [
  { key: 'users', label: t('school.exportUsers'), desc: t('school.exportUsersDesc') },
  { key: 'classes', label: t('school.exportClasses'), desc: t('school.exportClassesDesc') },
  { key: 'reports', label: t('school.exportReports'), desc: t('school.exportReportsDesc') },
  { key: 'activity', label: t('school.exportActivity'), desc: t('school.exportActivityDesc') },
];
</script>

<template>
  <div class="export-section">
    <h3>{{ t('school.exportData') }}</h3>
    <div class="export-grid">
      <div v-for="item in types" :key="item.key" class="export-card">
        ...
        <button class="btn-export" :disabled="exporting === item.key" @click="doExport(item.key)">
          {{ exporting === item.key ? '⏳ ...' : t('school.downloadCsv') }}
        </button>
      </div>
    </div>
  </div>

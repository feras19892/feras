...
export function useBoylesLawLayout() {
  const { t } = useI18n()

  return useExperimentLayout({
    storageKey: 'boyleslaw_layout_v2',
    clearOldKey: 'boyleslaw_layout_v1',
    panelIds: ['readings', 'chart', 'trials', 'params', 'laws', 'results'],
    defaultVisible: { readings: false, chart: true, trials: true, params: true, laws: true, results: false },
    defaultPanelColumn: { readings: 'data', chart: 'data', trials: 'data', params: 'ctrl', laws: 'ctrl', results: 'ctrl' },
    defaultColumnOrder: { data: ['chart', 'trials'], vis: [], ctrl: ['params', 'laws'] },

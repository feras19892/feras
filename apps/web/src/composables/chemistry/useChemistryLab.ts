...
// ================== GETTERS ==================
export {
  getLiquid, getBurette, getPipette, getSepFunnelState,
  getBurnerState, getBalanceTare, getContainerTare, getItemZoom,
  getBeakerClampState, getHotPlateState, getWoodenBaseState, getRetortStandState,
} from './useChemistryGetters';
import {
  getLiquid, getBurette, getPipette, getSepFunnelState,
  getBurnerState, getBalanceTare, getItemZoom,
} from './useChemistryGetters';
...
export function createLabItem(def: ToolDef, x: number, y: number): LabItem {
  ...
  if (isBurette(def.id)) {
    buretteMap[item.uid] = { volume: 50, maxVolume: 50, valveOpen: false, color: '#ef4444', opacity: 0.35, chemicalId: undefined, label: '' };
  }
  if (isPipette(def.id)) {

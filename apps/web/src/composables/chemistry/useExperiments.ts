import { items, liquidMap, buretteMap } from './useChemistryLab';
import { isContainer, isBurette } from './chemLabIds';
...
const validators: Record<string, (experiment: Experiment) => boolean[]> = {
  'neutralization-hcl-naoh': (exp) => {
    ...
    // Step 2: NaOH in burette
    c[1] = hasChemicalIn(isBurette, 'naoh');
    // Step 3: HCl in any beaker/container
    c[2] = hasChemicalIn(isContainer, 'hcl');
    // Step 4: Phenolphthalein in test tube
    c[3] = hasChemicalIn(isContainer, 'phenolphthalein');
    // Step 5: Indicator transferred to beaker (indicator + acid present in same container)
    c[4] = items.value.some((item) => {
      if (!isContainer(item.id)) return false;
      const liq = liquidMap[item.uid];
      if (!liq || liq.volume <= 0) return false;
      // Case A: beaker has HCl as main chemical + phenolphthalein in indicators
      const hasHclAsBase = liq.chemicalId === 'hcl';
      const hasIndicatorInArray = liq.indicators && liq.indicators.includes('phenolphthalein');
      // Case B: beaker has both in reactants tracking
      const hasHclInReactants = liq.reactants && liq.reactants['hcl'] > 0;
      const hasPhenInReactants = liq.reactants && liq.reactants['phenolphthalein'] > 0;
      // Case C: label shows both (fallback for mixed sources)
      const labelShowsBoth = liq.label && (liq.label.includes('كلور') || liq.label.includes('HCl')) && liq.label.includes('فينوفتالين');
      return (hasHclAsBase && hasIndicatorInArray) || (hasHclInReactants && hasPhenInReactants) || labelShowsBoth;
    });
    // Step 6: Burette valve is open
    c[5] = hasValveOpenBurette('naoh');
    // Step 7: Color changed to pink (pH > 8.2 with phenolphthalein)
    c[6] = items.value.some((item) => {
      if (!isContainer(item.id)) return false;
      const liq = liquidMap[item.uid];
      return liq && liq.ph !== null && liq.ph > 8.2 && liq.indicators && liq.indicators.includes('phenolphthalein');
    });

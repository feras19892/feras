...
import {
  ...
  isWatchGlass, isFilterFunnel, isRubberStopper, isContainer
} from './chemLabIds';
...
export function buildToolState(item: LabItem | null) {
  ...
  if (isContainer(item.id)) {
    ...
    return { uid: item.uid, type: 'beaker' as const, volume: liq.volume, maxVolume: liq.maxVolume, color: liq.color, opacity: liq.opacity, label: liq.label, stirred: liq.stirred, temperature: liq.temperature, ph: liq.ph, heated: liq.heated, viscosity: liq.viscosity, density: liq.density, surfaceTension: liq.surfaceTension, chemicalId: liq.chemicalId, indicators: liq.indicators, baseColor: liq.baseColor };
  }

...
const SAVE_KEY = 'grating:trials:v1'
...
export function useGratingTrials(
  ...
  function exportCsv(): string {
    const headers = ['Trial', 'N(lines/mm)', 'D(m)', 'lambda(nm)', 'maxOrder', 'theta1(deg)', 'y1(mm)']
    const rows = trials.value.map(t =>
      `${t.id},${t.linesPerMm},${t.screenDistance.toFixed(2)},${t.wavelength},${t.maxOrder},${t.firstOrderAngle.toFixed(3)},${t.firstOrderY.toFixed(3)}`
    )

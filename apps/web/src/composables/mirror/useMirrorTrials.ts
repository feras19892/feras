...
const SAVE_KEY = 'mirror:trials:v1'
...
export function useMirrorTrials(
  ...
  function autoLoad() {
    try {
      ...
      if (Array.isArray(parsed.trials)) {
        ...
        nextId.value = typeof parsed.nextId === 'number' ? parsed.nextId : (trials.value.length > 0 ? Math.max(...trials.value.map((t: MirrorTrial) => t.id)) + 1 : 1)
  ...
  function exportCsv(): string {
    ...
    const rows = trials.value.map(
      (t) => `${t.id},${t.mirrorType},${t.focalLength.toFixed(1)},${t.objectDistance.toFixed(1)},${t.objectHeight.toFixed(1)},${t.imageDistance.toFixed(1)},${t.imageHeight.toFixed(1)},${t.magnification.toFixed(3)},${t.invDo.toFixed(4)},${t.invDi.toFixed(4)}`
    )

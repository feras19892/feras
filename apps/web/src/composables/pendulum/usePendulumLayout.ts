...
export type PanelId = 'table' | 'equations' | 'error' | 'scatter' | 'tutor' | 'signal' | 'fft' | 'phase' | 'coupled' | 'params' | 'guide' | 'stats' | 'report'
...
const defaultPanelColumn: Record<PanelId, ColumnId> = {
  ...
  signal: 'vis', fft: 'vis', phase: 'vis', coupled: 'vis',
...
export function usePendulumLayout() {
  const panels = reactive<Record<PanelId, boolean>>({
    ...
    fft: true, phase: true, coupled: true, scatter: true, tutor: true, error: true, report: false,
  })
  ...
  const maximized = reactive<Record<PanelId, boolean>>({
    ...
    fft: false, phase: false, coupled: false, scatter: false, tutor: false, error: false, report: false,
  })
  ...
  function panelTitle(id: PanelId) {
    const titles: Record<PanelId, string> = {
      ...
      coupled: '🔗 بندول مقترن', params: '⚙️ معاملات', guide: '📋 دليل', stats: '📊 إحصائيات',
    }

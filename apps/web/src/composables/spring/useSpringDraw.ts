...
export function useSpringDraw(
  ...
  function draw() {
    ...
    const natLenPx = 50  // shorter natural spring for visual fit
    ...
    // Visual scale first (fit static stretch in upper 75% of canvas)
    const basePxPerCm = 8
    const pxPerCm = (natEqY + staticStretchCm * basePxPerCm > h * 0.75 && staticStretchCm > 0.001)
      ? Math.max(4, (h * 0.75 - natEqY) / staticStretchCm)
      : basePxPerCm

    // Oscillation scale: meters -> pixels (via pxPerCm). x[m]*100 = cm, *pxPerCm = pixels
    ...
    // Minimal ruler background (just enough for ticks)
    ...
    // Digital readouts on right side of ruler
    ...
    // Natural position (blue): always 0
    ...
    if (hasMass) {
      ...
      // Current position (mass): total position on ruler from natural (0)

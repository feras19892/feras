...
import type { GeneticsStage } from '../../../services/genetics-stages-data';
...
/** واجهة محرك المراحل الوراثية (Mitosis / Meiosis / DNA) */
export interface GeneticsEngine {
  currentStageIndex: Ref<number>;
...
const props = defineProps<{
  /** مصنع المحرك — يربط الحاوية بمحرك التجربة */
  engineFactory: (containerRef: Ref<HTMLDivElement | null>) => GeneticsEngine;
  stages: GeneticsStage[];
  experimentMeta: { titleKey: string; subtitleKey: string };
...
const {
  currentStageIndex, setStage, error, isLoading,
  autoRotate, toggleAutoRotate, resetCamera, resetAll, screenshot,
} = props.engineFactory(containerRef);

const tracking = useBiologyTracking();
watch(currentStageIndex, (idx) => {
  const s = props.stages[idx];
  if (!s) return;
  tracking.trackStage(s.id);
  tracking.trackPart(s.id, t(s.titleKey));
}, { immediate: true });

const stage = computed(() => props.stages[currentStageIndex.value]);
...
const stageItems = computed(() =>
  props.stages.map((s) => ({ id: s.id, label: t(s.titleKey), description: t(s.descriptionKey) }))
);
...
const goals = computed(() => [
  {
    ...
    completed: visitedStageIds.value.size >= props.stages.length,
  },
]);
...
const isLastStage = computed(() => currentStageIndex.value === props.stages.length - 1);
...
<template>
  <div class="experiment-page">
    <header class="experiment-header">
      ...
      <div class="header-content">
        <h1 class="experiment-title">{{ t(props.experimentMeta.titleKey) }}</h1>
        <p class="experiment-subtitle">{{ t(props.experimentMeta.subtitleKey) }}</p>
      </div>
      <BiologyReportButton
        :experiment-id="experimentId"
        :experiment-name="t(props.experimentMeta.titleKey)"
        :tracking="tracking"
      ...
      <aside class="side-panel parts-side">
        <div class="parts-card">
          <h2 class="panel-title">
            ...
            {{ t(props.stageLabelKey) }}
          </h2>
          <StageStepper
            :model-value="currentStageIndex"
            :stages="stageItems"
            :aria-label="t(props.stageLabelKey)"
  ...
  <BiologyHelpModal
    :open="helpOpen"
    :context="{ topic: experimentId ?? '', titleKey: props.experimentMeta.titleKey, subtitleKey: props.experimentMeta.subtitleKey, parts: props.stages.map((s) => ({ id: s.id, nameKey: s.titleKey })) }"

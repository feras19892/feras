...
const { currentStageIndex, setStage } = useProteinSynthesis3D(containerRef);
...
const stageLabels = computed(() =>
  proteinSynthesisStages.map((s, index) => ({
    id: s.id,
    label: t(s.titleKey),
    index,
  }))
    ...
    <main class="experiment-body">
      ...
      <section class="controls-section">
        ...
        <div class="action-bar">
          ...
          <button
            class="action-button secondary"
            :disabled="currentStageIndex === proteinSynthesisStages.length - 1"
            @click="nextStage"
          >
            {{ t('biology.nextStage') }}
          </button>

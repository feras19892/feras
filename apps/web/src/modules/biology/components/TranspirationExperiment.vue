...
import { useTranspiration3D } from '../../../composables/biology/useTranspiration3D';
import { useBiologyTracking } from '../../../composables/biology/useBiologyTracking';
import { transpirationStages, transpirationExperiment } from '../../../services/transpiration-data';
...
const { currentStageIndex, setStage, error, isLoading, autoRotate, toggleAutoRotate, resetCamera, resetAll, screenshot } = useTranspiration3D(containerRef);

// تتبّع المراحل التي وصل إليها الطالب لتغذية التقرير الحقيقي
const tracking = useBiologyTracking();
watch(currentStageIndex, (idx) => {
  const s = transpirationStages[idx];
  if (!s) return;
  tracking.trackStage(s.id);
  tracking.trackPart(s.id, t(s.titleKey));
}, { immediate: true });

const stage = computed(() => transpirationStages[currentStageIndex.value]);
...
const goals = computed(() => [
  {
    ...
    completed: visitedStageIds.value.size >= transpirationStages.length,
  },
]);
...
const isLastStage = computed(() => currentStageIndex.value === transpirationStages.length - 1);
...
<template>
  <div class="experiment-page">
    <header class="experiment-header">
      ...
      <div class="header-content">
        <h1 class="experiment-title">{{ t(transpirationExperiment.titleKey) }}</h1>
        <p class="experiment-subtitle">{{ t(transpirationExperiment.subtitleKey) }}</p>
      </div>
      <BiologyReportButton
        :experiment-id="experimentId"
        :experiment-name="t(transpirationExperiment.titleKey)"
        :tracking="tracking"
      ...
      <aside class="side-panel parts-side">
        <div class="parts-card">
          <h2 class="panel-title">
            ...
            {{ t('biology.transpirationStageLabel') }}
          </h2>
          <StageStepper
            :model-value="currentStageIndex"
            :stages="stageItems"
            :aria-label="t('biology.transpirationStageLabel')"
  ...
  <BiologyHelpModal
    :open="helpOpen"
    :context="{ topic: experimentId ?? '', titleKey: transpirationExperiment.titleKey, subtitleKey: transpirationExperiment.subtitleKey, parts: transpirationStages.map((s) => ({ id: s.id, nameKey: s.titleKey })) }"

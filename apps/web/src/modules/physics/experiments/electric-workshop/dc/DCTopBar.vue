  ...
  <div class="dc-top-bar">
    ...
    <template v-if="showValueEditor && editingComp">
      ...
      <div class="tb-section" v-if="editingComp?.type === 'resistor'">
        <button class="tb-action-btn" @click="emit('showResistorTutorial')">{{ String.fromCharCode(0x1F3A8) }} {{ t('ew.learnResistorColors') }}</button>
      </div>

      <div class="tb-section">
        <label class="tb-field">{{ t('ew.rotation') }}</label>
        <div class="tb-btn-grp">
          <button class="tb-mini-btn" @click="emit('update:editRotation', (editRotation + 270) % 360); emit('applyRotate')">{{ String.fromCharCode(0x21BA) }}</button>
          <span class="tb-rot-val">{{ editRotation }}{{ String.fromCharCode(0xB0) }}</span>
          <button class="tb-mini-btn" @click="emit('update:editRotation', (editRotation + 90) % 360); emit('applyRotate')">{{ String.fromCharCode(0x21BB) }}</button>
        </div>
      </div>
      ...
      <div class="tb-section">
        ...
        <div class="tb-btn-grp">
          <button class="tb-mini-btn" @click="emit('zoomComp', -0.2)">{{ String.fromCharCode(0x2212) }}</button>
      ...
      <div class="tb-section" v-if="editingComp?.type === 'switch'">
        <button class="tb-action-btn" @click="workshop.toggleSwitch(editingComp!.id); redraw()">
          {{ editingComp?.closed ? (String.fromCharCode(0x1F7E2) + ' ' + t('ew.on')) : (String.fromCharCode(0x1F534) + ' ' + t('ew.off')) }}
        </button>
      </div>
      ...
      <div class="tb-section" v-if="editingComp?.type === 'multimeter'">
        <div class="tb-mm-grp">
          ...
          <button class="tb-mm-btn" :class="{ active: editingComp?.multimeterMode === 'resistance' }" @click="workshop.setMultimeterMode(editingComp!.id, 'resistance')">{{ String.fromCharCode(0x3A9) }}</button>
        </div>
      </div>
      ...
      <div class="tb-section tb-actions">
        <button class="tb-delete-btn" @click="emit('deleteSelectedComp')">{{ String.fromCharCode(0x1F5D1) }} {{ t('ew.delete') }}</button>
      </div>
    </template>
    ...
    <template v-else-if="showWireEditor && editingWire">
      <div class="tb-section tb-name">
        <span class="tb-icon">{{ String.fromCharCode(0x1F9F5) }}</span>
      ...
      <div class="tb-section tb-actions">
        <button class="tb-delete-btn" @click="emit('deleteSelectedWire')">{{ String.fromCharCode(0x1F5D1) }} {{ t('ew.delete') }}</button>
      </div>
    </template>
    ...
    <template v-else>
      <div class="tb-section tb-name">
        <span class="tb-icon">{{ String.fromCharCode(0x2699, 0xFE0F) }}</span>

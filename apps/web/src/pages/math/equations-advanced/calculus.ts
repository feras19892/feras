...
export const equationsAdvanced_calculus: Equation[] = [
{
    id: 'derivative-sin',
    branchId: 'calculus',
    name: 'مشتقة الجيب',
  ...
  },
{
    ...
    solve(values) {
      const x = Number(values.x);
      if (Number.isNaN(x)) return { result: 'أدخل رقماً صحيحاً', steps: [] };
      if (x <= 0) return { result: 'x يجب أن يكون موجباًً', steps: [] };
  ...
  },
{
    ...
    description: 'تستخدم لاشتقاق دالة مركبة (دالة بداخل دالة).',
    ...
    applicationProblems: [
      {
        question: 'اشتقق (3x² + 1)ف.',
        hint: 'f(u)=uف، u=3x²+1، f′=5uف، u′=6x.',
        answer: '30x(3x²+1)ف',
  ...
  },
{
    ...
    description: 'تستخدم لاشتقاق حاصل ضرب دالتين من المتغير x.',
  ...
  },
{
    ...
    description: 'تستخدم لاشتقاق خارج قسمة دالتين. الناتج هو دالة أخرى قابلة للاشتقاق.',
    method: '1) حدد u(x) و v(x). 2) اشتق كل منهما للحصول على u′ و v′. 3) طبق القاعدة: (u′v - uv′) / v².',
    examples: [{ title: 'مثال', values: { a: 2, b: 3 }, steps: ['d/dx(2x / 3x²) = (2*3x² - 2x*6x)/(3x²)² = -6x/9xف = -2/(3x³)'] }],
    variables: [{ name: 'a', label: 'a' }, { name: 'b', label: 'b' }],
    solve(values) {
      const a = Number(values.a), b = Number(values.b);
      if ([a, b].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    description: 'يستخدم لإيجاد تكامل دالة الجيب بالنسبة لـ x.',
  ...
  },
{
    ...
    description: 'يستخدم لإيجاد تكامل دالة جيب التمام. x بالراديان.',
  ...
  },
{
    ...
    description: 'تمثل معدل تغير المشتقة الأولى. تستخدم لتحديد تقعر المنحنى والنقاط الحرجة.',
  ...
  },
{
    ...
    description: 'تستخدم لتكامل حاصل ضرب دالتين بتحويلها إلى دالة أبسط. أساسية في حساب التكاملات المعقدة.',

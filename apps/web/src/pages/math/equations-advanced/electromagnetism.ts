...
export const equationsAdvanced_electromagnetism: Equation[] = [
{
    id: 'magnetic-force',
    branchId: 'electromagnetism',
    name: 'قوة المجال المغناطيسي',
    formula: 'F = qvB',
    description: 'تستخدم لحساب القوة على شحنة متحركة في مجال مغناطيسي.',
    method: 'F = qvB.',
    ...
    solve(values) {
      ...
      if ([q, v, B].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    formula: 'B = μ0I / (2πr)',
    description: 'يستخدم لحساب المجال المغناطيسي حول سلك مستقيم.',
    method: 'B = μ0I / (2πr).',
    ...
    constants: [
      { label: 'μ₀', value: '4π × 10فف T·m/A', description: 'نفاذية الفراغ' },
    ],
  ...
  },
{
    ...
    name: 'مجال ملف دائري',
    formula: 'B = μ0NI / (2R)',
    description: 'يستخدم لحساب المجال المغناطيسي في مركز ملف دائري.',
    method: 'B = μ0NI / (2R).',
    ...
    solve(values) {
      ...
      if ([N, I, R].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
      if (R === 0) return { result: 'R لا يمكن أن يكون صفراً', steps: [] };
  ...
  },
{
    ...
    solve(values) {
      ...
      if ([B, A, theta].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    solve(values) {
      ...
      if ([V1, N1, N2].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    solve(values) {
      ...
      if ([L, dI, dt].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };

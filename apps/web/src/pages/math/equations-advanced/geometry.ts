...
export const equationsAdvanced_geometry: Equation[] = [
{
    id: 'area-trapezoid',
    branchId: 'geometry',
    name: 'مساحة شبه المنحرفف',
    formula: 'A = (a + b)/2 * h',
    description: 'يحسب مساحة شبه المنحرفف باستخدام طولي القاعدتين والارتفاع.',
    ...
    solve(values) {
      ...
      if ([a, b, h].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    solve(values) {
      ...
      if ([theta, r].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    solve(values) {
      ...
      if ([r, l].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    solve(values) {
      ...
      if ([h, k, r].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
      const result = r * r;
  ...
  },
{
    ...
    solve(values) {
      ...
      if ([A, B, C, x0, y0].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    solve(values) {
      ...
      if ([n, s].some((x) => Number.isNaN(x))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };

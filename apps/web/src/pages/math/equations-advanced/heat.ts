...
export const equationsAdvanced_heat: Equation[] = [
{
    id: 'heat-capacity',
    branchId: 'heat',
    name: 'السعة الحرارية',
    formula: 'Q = mcΔT',
    description: 'تستخدم لحساب الحرارة اللازمة لتغيير درجة الحرارة.',
    method: 'Q = mcΔT.',
    ...
    solve(values) {
      ...
      if ([m, c, deltaT].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    formula: 'PV = nRT',
    description: 'يستخدم لحساب الضغط أو الحجم أو درجة الحرارة للغاز المثالي.',
    method: 'P = nRT / V.',
    ...
    solve(values) {
      ...
      if ([n, V, T].some((x) => Number.isNaN(x))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    formula: 'Q = mL',
    description: 'تستخدم لحساب الحرارة اللازمة لتغيير حالة المادة.',
    method: 'Q = mL.',
    ...
    solve(values) {
      ...
      if ([m, L].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    name: 'التمدد الحراري',
    formula: 'ΔL = αL0ΔT',
    description: 'يستخدم لحساب التغير في الطول بسبب التغير في الحرارة.',
    method: 'ΔL = αL0ΔT.',
    ...
    solve(values) {
      ...
      if ([alpha, L0, deltaT].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    formula: 'ΔU = Q - W',
    description: 'يستخدم لحساب التغير في الطاقة الداخلية للنظام.',
    method: 'ΔU = Q - W.',
    ...
    solve(values) {
      ...
      if ([Q, W].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    solve(values) {
      ...
      if ([Qc, Qh].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    solve(values) {
      ...
      if ([P1, V1, P2].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    solve(values) {
      ...
      if ([V1, T1, T2].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    solve(values) {
      ...
      if ([P1, T1, T2].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
      if (T1 === 0) return { result: 'T1 يجب ألا يكون صفراً', steps: [] };
      const P2 = (P1 * T2) / T1;

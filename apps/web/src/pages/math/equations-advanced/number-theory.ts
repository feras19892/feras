...
export const equationsAdvanced_number_theory: Equation[] = [
{
    id: 'gcd',
    branchId: 'number-theory',
    name: 'القاسم المشترك الأكبر',
    formula: 'gcd(a, b)',
    description: 'يستخدم لإيجاد أكبر عدد يقسم a و b بدون باق.',
  ...
  },
{
    ...
    description: 'يستخدم لإيجاد أصغر عدد يقبل القسمة على a و b.',
  ...
  },
{
    ...
    formula: 'S = n(n + 1) / 2',
    description: 'يستخدم لحساب مجموع الأعداد من 1 إلى n.',
    method: 'S = n(n + 1) / 2.',
  ...
  },
{
    ...
    description: 'يستخدم لإيجاد الحد nth في متتالية حسابية.',
    ...
    variables: [{ name: 'a1', label: 'aف' }, { name: 'd', label: 'd' }, { name: 'n', label: 'n' }],
    solve(values) {
      const a1 = Number(values.a1), d = Number(values.d), n = Number(values.n);
      if ([a1, d, n].some((x) => Number.isNaN(x))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    description: 'يستخدم لإيجاد الحد n في متتالية هندسية، حيث كل حد ينتج بضرب الحد السابق في أساس r.',
    ...
    variables: [{ name: 'a1', label: 'aف' }, { name: 'r', label: 'r' }, { name: 'n', label: 'n' }],
    solve(values) {
      const a1 = Number(values.a1), r = Number(values.r), n = Number(values.n);
      if ([a1, r, n].some((x) => Number.isNaN(x))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };
  ...
  },
{
    ...
    description: 'يستخدم لحساب مجموع أول n حدود في متتالية هندسية عندما لا يكون الأساس يساوي 1.',
  ...
  },
{
    ...
    name: 'المضروب',
    formula: 'n! = n × (n-1) × ... × 1',
    description: 'يستخدم لحساب عدد الطرق الممكنة لترتيب n عناصر.',
  ...
  },
{
    ...
    formula: 'a mod m = r',
    description: 'يحسب باقي قسمة a على m. يستخدم في التشفير، جداول الأوقات، والنماذج الدورية.',
    ...
    solve(values) {
      ...
      if ([a, m].some((n) => Number.isNaN(n))) return { result: 'أدخل أرقاماًً صحيحة', steps: [] };

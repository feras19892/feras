...
import type { TeacherEvaluation } from '@/services/school-reports.service'
import SkeletonLoader from '@/components/shared/SkeletonLoader.vue'

const props = defineProps<{
  detailData: SchoolUserDetailResult | null
  detailLoading: boolean
  teacher: TeacherEvaluation
}>()
...
<template>
  <div class="teacher-expand" @click.stop>
    <SkeletonLoader v-if="detailLoading" type="table" :count="3" />
    <div v-else-if="detailData" class="teacher-detail">
      <div class="teacher-profile">
        ...
        <div class="teacher-profile__body">
          ...
          <SchoolHelpButton tab-id="teacher-detail" />
      ...
      <div v-if="activeTab === 'overview'" class="tab-panel">
        <div class="stat-cards-grid">
          ...
          <div class="stat-card-modern">
            <div class="stat-card-modern__icon">⭐</div>
            <div class="stat-card-modern__body">
              <div class="stat-card-modern__value">{{ Math.round(teacher.teaching_score) }}</div>
      ...
      <div v-if="activeTab === 'classes'" class="tab-panel">
        <div v-if="detailData.taughtClasses.length" class="detail-table-wrap">
          <table class="detail-table">
            <thead><tr><th>الفصل</th><th>الطلاب</th><th>الحالة</th></tr></thead>
            <tbody><tr v-for="c in detailData.taughtClasses" :key="c.id"><td>{{ c.name }}</td><td>{{ c.student_count ?? 0 }}</td><td>{{ c.is_frozen ? '🔒 مجمد' : '✅ نشط' }}</td></tr></tbody>
          </table>
        </div>
      ...
      <div v-if="activeTab === 'reports'" class="tab-panel">
        <div v-if="detailData.reports.length" class="detail-table-wrap">
          <table class="detail-table">
            <thead><tr><th>التجربة</th><th>الطالب</th><th>الفصل</th><th>الحالة</th><th>العلامة</th><th>التاريخ</th></tr></thead>
            <tbody><tr v-for="r in detailData.reports" :key="r.id"><td>{{ r.experiment_name }}</td><td>{{ r.student_name }}</td><td>{{ r.class_name ?? '—' }}</td><td>{{ r.status }}</td><td>{{ r.grade ?? '—' }}</td><td>{{ formatDate(r.submitted_at || r.created_at) }}</td></tr></tbody>
          </table>
        </div>
...
.teacher-detail { padding: 16px; border-top: 1px dashed var(--border-color); }
.teacher-profile { display: flex; align-items: center; gap: 16px; margin-bottom: 16px; }
.avatar--lg { width: 64px; height: 64px; font-size: 28px; }
.teacher-profile__body h4 { display: flex; align-items: center; gap: 8px; margin: 0 0 6px; font-size: 18px; }

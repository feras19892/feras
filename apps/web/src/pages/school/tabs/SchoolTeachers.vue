<template>
  <div class="dash-page">
    ...
    <div class="toolbar-right">
      <button @click="showAddModal = true" class="btn-add">➕ مدرس جديد</button>
    ...
    <div v-if="teachers.length" class="class-toolbar">
      <button class="toolbar-btn" :disabled="!activeTeacher" @click="onActionActive('detail')">👁️ تفاصيل</button>
      <button class="toolbar-btn" :disabled="!activeTeacher" @click="onActionActive('warn')">⚠️ تحذير</button>
      <button class="toolbar-btn" :disabled="!activeTeacher" @click="onActionActive('report')">📢 بلاغ أدمن</button>
      <button class="toolbar-btn" :disabled="!activeTeacher" @click="onActionActive('alert')">🔔 إرسال تنبيه</button>
      <button v-if="activeTeacher?.is_blocked" class="toolbar-btn" :disabled="!activeTeacher" @click="onActionActive('unblock')">✅ إلغاء الحظر</button>
      <button v-else class="toolbar-btn" :disabled="!activeTeacher" @click="onActionActive('block')">🚫 حظر</button>
      <button class="toolbar-btn toolbar-danger" :disabled="!activeTeacher" @click="onActionActive('delete')">🗑️ حذف</button>
    </div>
    <div v-if="teachers.length" class="compact-list">
      <div
        v-for="t in pagedTeachers" :key="t.id"
        class="compact-row"
        :class="{ 'row-selected': activeTeacher?.id === t.id, 'class-frozen': t.is_blocked }"
        @click="selectTeacher(t)"
      >
        <span class="cr-icon">{{ t.is_blocked ? '🚫' : '👨‍🏫' }}</span>
        <span class="cr-name">{{ t.name }}<span v-if="t.is_blocked" class="frozen-badge">محظور</span></span>
        <span class="cr-meta">
          <span>فصول: {{ t.class_count }}</span>
          <span>طلاب: {{ t.total_students }}</span>
          <span>تقارير: {{ t.total_reports }}</span>
          <span :class="['st-rate', gradingClass(t)]">تصحيح: {{ Math.round(t.grading_rate) }}%</span>
          <span :class="['st-score', scoreClass(t)]">أداء: {{ Math.round(t.teaching_score) }}</span>
        </span>
        <span class="cr-action" @click.stop>
          <span class="st-email">{{ t.email }}</span>
        </span>
        <SchoolTeacherDetail v-if="expandedTeacher?.id === t.id" :detail-data="detailData" :detail-loading="detailLoading" :teacher="t" />
      </div>
    </div>
    <Pagination
    ...
    />
    <EmptyState v-else icon="👨‍🏫" title="لا يوجد مدرسون" />
    ...
    <ConfirmModal
      ...
      icon="🚫"
      title="تأكيد الحظر"
      :message="blockTarget ? `هل تريد حظر المدرس ${blockTarget.name}؟ لن يتمكن من الدخول للنظام.` : ''"
    ...
    <ConfirmModal
      ...
      icon="⚠️"
      title="تحذير نهائي"
      :message="blockTarget ? `سيتم حظر ${blockTarget.name} فوراً. هل أنت متأكد تماماً؟` : ''"
    ...
    <ConfirmModal
      ...
      icon="🗑️"
      title="تأكيد الحذف"
      :message="deleteTarget ? `هل تريد حذف المدرس ${deleteTarget.name}؟ سيتم حذف جميع بياناته نهائياً.` : ''"
      confirm-label="متابعة"
      cancel-label="إلغاء"
      variant="danger"
    ...
    <ConfirmModal
      ...
      icon="⚠️"
      title="تحذير نهائي"
      :message="deleteTarget ? `تحذير: سيتم حذف ${deleteTarget.name} وكل ما يتعلق به نهائياً! لا يمكن التراجع عن هذه العملية. هل أنت متأكد تماماً؟` : ''"
...
import { useSchoolTeachers } from '@/composables/school/useSchoolTeachers'
import SkeletonLoader from '@/components/shared/SkeletonLoader.vue'
import ErrorState from '@/components/shared/ErrorState.vue'
import EmptyState from '@/components/shared/EmptyState.vue'
import ConfirmModal from '@/components/shared/ConfirmModal.vue'
import Pagination from '@/components/shared/Pagination.vue'
import SchoolTeacherDetail from './SchoolTeacherDetail.vue'
...
const {
  ...
  selectTeacher, onActionActive, loadDetail, gradingClass, scoreClass,
...
.expand-content { display: flex; flex-direction: column; gap: 12px; }
.expand-title { margin: 4px 0 6px; font-size: 13px; font-weight: 700; color: var(--text-primary); }

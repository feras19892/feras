<template>
  <div class="chat-page">
    ...
    <ErrorState v-else-if="store.error" :error="store.error || ''" show-retry @retry="store.fetchClasses(true)" />
    <template v-else>
      <div v-if="classes.length" class="class-circles">
        <button
          v-for="c in classes"
          :key="c.id"
          class="class-circle"
      ...
      <ClassChat
        ...
        class="chat-room"
      />
      <EmptyState v-else-if="classes.length" icon="💬" title="اختر فصلاً" />
    </template>
  </div>
</template>
